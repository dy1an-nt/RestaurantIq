...
5. You reproduced it in the running app (`curl` against the endpoint, the page loaded in the browser).

Any safety fact you can't get to step 4, say so out loud. Don't write it up as settled. Step 4 is usually one small script or one `npm test -- <pattern>` run in `restaurantiq-backend/` that calls the exact function you're worried about.

This is the same standard as Operating Discipline gate 2: exercised, not just compiled.
...
2. Find the one fact it's safe because of. Most changes that look scary are safe because of a single fact, like "this call only drops already-expired token rows and does nothing else". Find that fact. If it holds, most of the scary cases die at once. Spend your time here, not on a long list of maybes.
3. Look where grep stops. Read the source of the library you call, and check its pinned version in `package.json`. Work out when things run: microtasks, React unmount and teardown, the sync scheduler's timers. Follow what a symbol search misses: the JSON shape a route returns, a DB column, a Square or DoorDash wire format, a feature flag, code three hops downstream. In this repo, check these four every time:
   - **Tenant scoping.** Does the change touch any query that must be filtered by `restaurantId` / `req.user`? A leak here is the worst failure this codebase has.
   - **Response shape.** Is `{ data, error }` still intact on every route in the diff, including error paths?
   - **Money.** Still integer cents everywhere, formatted only at display?
   - **Migrations.** Numbered and idempotent, and safe to run against a database that already has partial data?
...
6. For a wide change, or anything touching auth, tenant scoping, or money, this skill is not enough on its own. Run a real `qa-agent` pass as well. That escalation is not negotiable (CLAUDE.md, lightweight path).
