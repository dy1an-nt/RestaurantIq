...
This file is the Codex entrypoint. It mirrors [`CLAUDE.md`](CLAUDE.md), the
Claude entrypoint, which adds a `@` import Codex has no equivalent for. The
block between the `shared:root-instructions` markers below must be
byte-identical in both files; `scripts/check-docs.mjs` fails CI if it drifts.
**Edit both, or neither.**

## Read this first

**Read [`RestaurantIQ/CLAUDE.md`](RestaurantIQ/CLAUDE.md) in full before your
first action in this repo.** It is the canonical project instruction file and it
binds the main session, not just subagents. The **Operating Discipline** section
applies in particular. Claude pulls it in with a `@` import; Codex does not, so
read it explicitly.
